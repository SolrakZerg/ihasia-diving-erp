...
for (const year in facturado) {
  for (const month in facturado[year]) {
    ...
    const s = (ssi[year] && ssi[year][month]) || 0;
    values.push(`(${year}, ${month}, ${f}, ${s}, 0)`);
  }
}

const query = `INSERT INTO monthly_reports (year, month, facturado, ssi_estimated, total_gastos) VALUES \n` + values.join(',\n') + ';';
