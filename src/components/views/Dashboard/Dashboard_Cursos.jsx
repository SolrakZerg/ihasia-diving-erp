export default function Dashboard_Cursos({ courseStats, year }) {
   ...
   const getGrowthBadge = (val) => {
      ...
      const isPos = val >= 0;
      return (
         <span className={`text-[16px] font-black px-2 py-0.5 rounded-md ${isPos ? 'bg-emerald-500/20 text-emerald-400' : 'bg-danger/20 text-danger'}`}>
            {isPos ? '+' : ''}{val}%
         </span>
      );
   };
   ...
   return (
      <div style={{ flex: '1 1 200px' }} className="bg-surface-soft border border-surface-edge rounded-3xl p-4 shadow-xl flex flex-col relative overflow-hidden h-[480px] max-md:w-full md:max-w-[300px]">
         ...
         <h3 className="text-[14px] font-black text-text-header uppercase tracking-[0.2em] mb-auto text-center z-10">CURSOS</h3>
         ...
         <div className="flex flex-col items-center justify-center flex-1 z-10">
            ...
            <div className="flex flex-col gap-2 w-full px-2">
               ...
               <div className="flex items-center justify-center gap-4 bg-surface-edge/10 border border-surface-edge/20 px-3 py-2.5 rounded-xl">
                  <span className="text-[14px] font-bold text-text-header uppercase tracking-widest">{year - 1}</span>
               ...
               {/* Comparativa 2024 */}
               <div className="flex items-center justify-center gap-4 bg-surface-edge/10 border border-surface-edge/20 px-3 py-2.5 rounded-xl">
                  <span className="text-[14px] font-bold text-text-header uppercase tracking-widest">{year - 2}</span>
                  <div className="flex items-center gap-2">
               ...
               {/* Comparativa 2023 */}
               <div className="flex items-center justify-center gap-4 bg-surface-edge/10 border border-surface-edge/20 px-3 py-2.5 rounded-xl">
                  <span className="text-[14px] font-bold text-text-header uppercase tracking-widest">{year - 3}</span>
                  <div className="flex items-center gap-2">
