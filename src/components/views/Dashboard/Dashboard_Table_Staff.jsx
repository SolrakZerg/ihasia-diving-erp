export default function Dashboard_Table_Staff({ staffData }) {
   ...
   return (
      <div style={{ flex: '1 1 250px' }} className="bg-surface-soft border border-surface-edge rounded-2xl p-4 shadow-xl h-[480px] flex flex-col max-md:w-full md:max-w-[350px]">
         <h3 className="text-[14px] font-black text-text-header uppercase tracking-[0.2em] mb-3 px-2 text-center">Staff</h3>
         <div className="flex-1 overflow-auto custom-scrollbar">
            <table className="w-full text-left">
               <thead>
                  <tr className="border-b border-surface-edge/50">
                     <th className="text-[14px] font-black text-text-header uppercase py-0.5">Nombre</th>
                     <th className="text-[14px] font-black text-text-header uppercase py-0.5 text-right">Sueldo</th>
                     <th className="text-[14px] font-black text-text-header uppercase py-0.5 text-right">Pend.</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-surface-edge/30">
                  {staffData.map((s, idx) => (
                     <tr key={idx} className="group hover:bg-surface-edge transition-colors">
                        <td className="py-1 text-sm font-black text-white/90 uppercase">{s.name}</td>
                        <td className="py-1 text-right text-sm font-mono text-white/90">{Math.round(s.totalEarned).toLocaleString()}</td>
               ...
               <tfoot className="sticky bottom-0 bg-surface-soft border-t-2 border-surface-edge/80 shadow-[0_-10px_20px_rgba(0,0,0,0.2)]">
                  <tr className="h-10">
                     <td className="text-[13px] font-black text-text-muted uppercase italic pl-2">Total</td>
