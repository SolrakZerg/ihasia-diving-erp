        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead className="md:sticky top-0 z-30 bg-table-header/98 backdrop-blur-xl h-[70px]">
            <tr className="border-b border-surface-edge">
              <th className="p-2 text-[10px] font-black text-text-header uppercase tracking-widest text-center w-12 bg-table-header/98 backdrop-blur-xl sticky left-0 z-40">Día</th>
              ...
              <th className="p-2 text-[14px] font-black text-white uppercase tracking-widest text-right bg-table-header/98 backdrop-blur-xl w-auto">Total</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-surface-edge/40">
            {Object.keys(matrixData).map(day => (
              <tr key={day} className="group hover:bg-white/5 transition-colors h-9">
                <td className="p-0 text-center font-black text-text-muted text-sm sticky left-0 bg-table-header/98 backdrop-blur-xl z-20 group-hover:bg-surface-edge/20 transition-colors border-r border-surface-edge/10">{day}</td>
          ...
          <tfoot className="md:sticky bottom-0 z-30 bg-table-header/98 backdrop-blur-xl border-t-2 border-surface-edge shadow-[0_-4px_10px_rgba(0,0,0,0.3)] h-9 font-black">
            <tr>
              <td className="p-0 text-center text-text-muted text-[10px] uppercase bg-table-header/98 backdrop-blur-xl sticky left-0 z-40 border-r border-surface-edge/10">TOT</td>
              ...
              <td className="p-1 text-right border-l border-surface-edge/20 text-emerald-400 text-lg bg-table-header/98 backdrop-blur-xl pr-4">{(totalComm + totalAssists + totalAdj).toLocaleString()}</td>
            </tr>
          </tfoot>
        </table>
      </div>
