      ...
      <div className="bg-[#0B1121]/95 backdrop-blur-2xl border border-white/10 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] rounded-2xl p-1.5 flex items-center gap-1">
        <div className="px-4 py-2 flex items-center gap-3 border-r border-white/5 mr-1">
          ...
          <div className="flex flex-col justify-center">
            <span className="text-white text-sm font-black leading-none">Seleccionados</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pr-1 ml-2">
          ...
          <div className="flex items-center bg-white/5 rounded-xl p-1 gap-1 border border-white/5">
            <button onClick={() => setBulkGroupAction(bulkGroupAction === 'group' ? null : 'group')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all font-black text-xs uppercase tracking-tight ${bulkGroupAction === 'group' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/40' : 'hover:bg-white/5 text-blue-400/60 hover:text-blue-400'}`}>
              <Link className="w-3.5 h-3.5" />Agrupar
            </button>
            <button onClick={() => setBulkGroupAction(bulkGroupAction === 'ungroup' ? null : 'ungroup')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all font-black text-xs uppercase tracking-tight ${bulkGroupAction === 'ungroup' ? 'bg-orange-600 text-white shadow-lg shadow-orange-900/40' : 'hover:bg-white/5 text-orange-400/60 hover:text-orange-400'}`}>
          ...
          <div className="relative h-10">
            <button className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all font-black text-sm group border h-full ${bulkDate ? 'bg-brand/20 border-brand/40 text-brand shadow-[0_0_15px_-3px_rgba(var(--brand-rgb),0.4)]' : 'hover:bg-white/5 border-transparent text-gray-400'}`} onClick={() => document.getElementById('bulk-date-input').showPicker()}>
          ...
          <div className={`flex items-center gap-2 rounded-xl px-3 border transition-all h-10 ${bulkInstructor ? 'bg-amber-500/10 border-amber-500/40 shadow-[0_0_15px_-3px_rgba(245,158,11,0.2)]' : 'bg-white/5 border-transparent'}`}>
            ...
            <select className="bg-transparent text-white text-sm font-black outline-none cursor-pointer pr-2 py-1.5" value={bulkInstructor} onChange={(e) => setBulkInstructor(e.target.value)}>
          ...
          {/* Apply */}
          <button onClick={handleApplyBulkChanges} disabled={loadingInvoices || (!bulkGroupAction && !bulkDate && !bulkInstructor)} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-30 disabled:hover:bg-emerald-600 text-white rounded-xl transition-all font-black text-sm shadow-lg shadow-emerald-900/20 active:scale-95 h-10">
