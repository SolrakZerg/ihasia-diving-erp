...
-- ================================================================================
-- SECTION: ROSTER 3.0 INTEGRATION FUNCTIONS (Integración ERP <-> Roster)
-- ================================================================================

-- --------------------------------------------------------------------------------
-- Función: public.get_roster_bookings_by_date(target_date date)
-- Propósito: Obtiene las reservas del día desde customers para importar al Roster.
-- Parámetros: target_date (date) - Fecha de buceo consultada.
-- Retorna: TABLE con datos limpios del cliente, actividad, nivel y tallas.
-- ERP Módulo: Modal de importación diaria en Roster.
-- --------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_roster_bookings_by_date(target_date date)
 RETURNS TABLE(id uuid, first_name text, last_name text, roster_name text, level text, booked_activity text, booking_date date, bcd_size text, suit_size text, fins_size text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
GRANT EXECUTE ON FUNCTION public.get_roster_bookings_by_date(date) TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.get_roster_bookings_by_date(date) IS 'Obtiene las reservas de clientes por fecha para importación en el Roster.';
...
-- Parámetros: query_text (text) - Texto tecleado en el autocompletado del Roster.
--             target_date (date) - Fecha activa del Roster para marcar con estrella (is_today).
-- Retorna: TABLE con campos optimizados y ranking de ordenación inteligente.
-- ERP Módulo: Autocompletado de alumnos en filas del Roster.
-- --------------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------------
-- Función: public.update_customer_gear(...)
-- Propósito: Actualiza las tallas de equipo (BCD, traje, aletas) de un cliente desde Roster.
--            Blindado: requiere UUID o nombre completo exacto (sin adivinar por iniciales).
--            Permite vaciar tallas si se selecciona '--' o vacío.
-- Parámetros: p_customer_id (UUID), p_customer_name (text), p_bcd (varchar), p_suit (varchar), p_fins (varchar)
-- ERP Módulo: Edición de tallas en Roster y sincronización con ficha de cliente en ERP.
-- --------------------------------------------------------------------------------
